( function( $ ) {
    ...
    $( '.instafeed-wrapper' ).each( function() {
        ...
        $.ajax({
            ...
            error: function (data) {
                var output = '<div class="col-12"><span class=text-center></span></div>';
                _this.append( output );
            }
        });
    });
    ...
    let products = [
        {
            name: 'Урт Түрүүтэй',
            price: 470000,
            imgSrc1: '{% static /images/ratio/product1.jpg %}',
            imgSrc2: '{% static /images/ratio/5.jpg %}',
            hasBoots: 'long',
            color: 'Бор',
            sex: 'man'
        },
        {
            name: 'Богино түрүүтэй',
            price: 570000,
            imgSrc1: '{% static /images/ratio/product1.jpg %}',
            imgSrc2: '{% static /images/ratio/5.jpg %}',
            hasBoots: 'short',
            ...
            ` <li class="grid-item wow animate__fadeIn" data-wow-delay="0.2s">
                    <div class="product-box margin-25px-bottom xs-margin-15px-bottom">
                        ...
                        <div class="product-footer text-center padding-25px-top xs-padding-10px-top">
                            <a href="{% url 'singleProduct' %}" class="text-extra-dark-gray font-weight-500 d-inline-block">${product.name} ${product.color}</a>
    ...
    var checkboxes_on_category = [false, false]
    function filterProducts(checkboxId = null) {
        if (checkboxId != null) {
            
            let checkboxValue = document.getElementById(checkboxId)

            if (checkboxValue.id == "long") {
                if (checkboxValue.checked) {
                    checkboxes_on_category[0] = true
                }
                else {
                    checkboxes_on_category[0] = false
                }
            }
            if (checkboxValue.id == "short") {
                if (checkboxValue.checked) {
                    checkboxes_on_category[1] = true
                }
                else {
                    checkboxes_on_category[1] = false
                }
            }
            if (checkboxValue.id == "bor"){
                if(checkboxValue.checked){
                    checkboxes_on_category[0] = true
                }else{
                    checkboxes_on_category[0] = false
        ...
        let values = [valuesArr[0].slice(1,valuesArr[0].length),valuesArr[1].slice(1,valuesArr[1].length)];
        let intValues = [parseInt(values[0]),parseInt(values[1])];
        let filteredElements = null
        if (!checkboxes_on_category[0] && !checkboxes_on_category[1]) {
            filteredElements = products.filter(product => product.price > intValues[0] && product.price < intValues[1]);
        }
        
        if (!checkboxes_on_category[0] && checkboxes_on_category[1]) {
            filteredElements = products.filter(product => product.price > intValues[0] && product.price < intValues[1] && product.hasBoots == 'short');
        }
        if (checkboxes_on_category[0] && !checkboxes_on_category[1]) {
            filteredElements = products.filter(product => product.price > intValues[0] && product.price < intValues[1] && product.hasBoots == 'long');
        }
        if (checkboxes_on_category[0] && !checkboxes_on_category[1]) {
            filteredElements = products.filter(product => product.price > intValues[0] && product.price < intValues[1] && product.sex == 'man');
        }
        if (!checkboxes_on_category[0] && checkboxes_on_category[1]) {
            filteredElements = products.filter(product => product.price > intValues[0] && product.price < intValues[1] && product.sex == 'woman');
        }
        ...
        // let filteredElements = products.filter(product => product.price > intValues[0] && product.price < intValues[1] && product.hasBoots == ''  );
        
        let filteredHtml = filteredElements.map(product => productTemplate(product));
